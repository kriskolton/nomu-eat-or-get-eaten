  ...
  async (req, res) => {
    try {
      ...
      try {
        ...
        console.error("verifyReplay threw an error", {
          userId,
          sessionId,
          error: error.message,
        });
        verifyResult = { ok: false, failedDueTo: "Verification crashed" };
      }
      ...
      if (!verifyResult.ok) {
        ...
        isFlagged = true;
        flaggedFor.push(verifyResult.failedDueTo);
      }
...
function setupBotCommands() {
  ...
  bot.onText(/\/play/, async (msg) => {
    ...
    await sendMessageWithErrorHandling(
      chatId,
      `Welcome to Nomu: Eat or Get Eaten ${activeEvent} ⚔️\n\n` +
